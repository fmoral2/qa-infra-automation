...
# Determine if this is the first server or additional server
if [[ "$has_etcd" == true || "$has_cp" == true ]]; then
  # This is a server node
  if [[ "${HOSTNAME}" == *"master"* ]] || [[ "${ansible_hostname:-$(hostname)}" == *"master"* ]]; then
    # First server - bootstrap the cluster
    echo "Configuring as first server (cluster-init)"
    config="$config
cluster-init: true"
  else
    # Additional server - join existing cluster
    echo "Configuring as additional server (join cluster)"
    config="$config
